...
```
code-sandbox/
├── firefoundry.json              # Application config (contains application_id)
├── apps/
│   └── coder-bundle/             # Your agent bundle
│       ├── firefoundry.json      # Bundle config (contains agent_bundle_id, port, health, resources)
│       ├── src/
...
Notice the two `firefoundry.json` files. The root one identifies the **application** -- the overall project that can contain multiple components (agent bundles, web apps, etc.). The nested one identifies this specific **agent bundle** -- one deployable component within the application. Both contain UUIDs generated by `ff-cli` during scaffolding.
...
// These IDs are generated by ff-cli and stored in firefoundry.json.
// APPLICATION_ID comes from the root firefoundry.json (the overall project).
// AGENT_BUNDLE_ID comes from the bundle's firefoundry.json (this specific component).
// ff-cli populates them into the scaffolded source code automatically.
const APPLICATION_ID = "a1b2c3d4-e5f6-7890-abcd-ef1234567890";
...
Two IDs, two different purposes:

- **`APPLICATION_ID`** identifies the parent application -- the `code-sandbox` project. An application can contain multiple components (agent bundles, web apps, workers). At startup, the SDK uses this to register the parent application in the platform.
- **`AGENT_BUNDLE_ID`** identifies this specific agent bundle -- `coder-bundle`. It's the component's own identity, used for entity graph scoping and telemetry.

Both are generated by `ff-cli` during scaffolding and stored in their respective `firefoundry.json` files. You don't need to generate UUIDs yourself.
